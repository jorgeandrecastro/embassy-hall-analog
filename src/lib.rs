...
//! basé sur le framework [Embassy](https://embassy.dev), ce n'est pas développé par l'équipe ooficille attention!!.
//!
...
//!         // ~2048 → pas de champ  |  > 2048 → pôle Sud  |  < 2048 → pôle Nord
//!         let _ = raw;
//!     }
//! }
//! ```
...
//!
//! La valeur brute ADC peut être convertie en tension (12 bits sur RP2040, 14 bits sur RP235x) :
//!
//! ```text
...
/// Utilisez [`HallAnalog::read_raw`] pour calibrer votre point zéro.
...
/// // let direction = sensor.read_polarity().await; // Positive, Negative ou NoField
/// ```
...
/// par rapport au point de repos (mi-échelle).
...
impl<'d> HallAnalog<'d> {
    ...
    ///
    /// Retourne la valeur brute lue :
    /// - `0..=4095` sur RP2040 (12 bits)
    /// - `0..=16383` sur RP235x (14 bits)
    ///
    /// La valeur est centrée autour de **~2048** (RP2040) ou **~8192** (RP235x)
    /// en l'absence de champ magnétique.
    ///
    ...
    ///
    /// Utilise une zone morte (`deadband`) configurable pour éviter les faux
    /// positifs dus au bruit ADC autour du point de repos.
    ///
    /// # Arguments
    ///
    /// * `deadband` Demi-largeur de la zone morte (en LSB ADC).
    ///   Une valeur typique est `50` sur 12 bits.
    ///   Si `raw > ZERO + deadband` → [`MagneticPolarity::SouthPole`]  
    ///   Si `raw < ZERO - deadband` → [`MagneticPolarity::NorthPole`]  
    ///   Sinon                       → [`MagneticPolarity::NoField`]
    ///
    /// # Retour
    ///
    /// * [`MagneticPolarity`] — Polarité du champ détecté.
    ///
    /// # Exemple
    ///
    /// ```rust,no_run
    ...
    pub async fn read_polarity(&mut self, deadband: u16) -> MagneticPolarity {
        ...
        let zero = ZERO_FIELD_RAW_12BIT;

        if raw > zero.saturating_add(deadband) {
            MagneticPolarity::SouthPole
        } else if raw < zero.saturating_sub(deadband) {
    ...
    /// # Arguments
    ///
    /// * `zero`  Point de repos calibré (utilisez [`ZERO_FIELD_RAW_12BIT`] ou
    ///   [`ZERO_FIELD_RAW_14BIT`] selon votre puce, ou une valeur mesurée).
    ///
    /// # Retour
    ///
    /// * `i32` Déviation signée en LSB. Positif = pôle Sud, négatif = pôle Nord.
    ///
    /// # Exemple
    ///
    /// ```rust,no_run
